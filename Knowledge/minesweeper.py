...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        """
        #marking the cell that has been made
        ...
        #remove all cells from neighbouring set, that have known states
        mine_count = count
        ...
        for neighbour in neighbours:
            ...
            if neighbour in self.safes:
                neighbours_to_remove.add(neighbour)
        neighbours -= neighbours_to_remove
        
        #add new sentence to knowledge base
        new = Sentence(neighbours, mine_count)
        self.knowledge.append(new)

        #marking any other cells as mine or safe
        mine_to_mark = set()
        safe_to_mark = set()

        for sentence in self.knowledge:
            for mine in sentence.known_mines():
                mine_to_mark.add(mine)
            for safe in sentence.known_safes():
                safe_to_mark.add(safe)
        for mine in mine_to_mark:
            self.mark_mine(mine)
        for safe in safe_to_mark:
            self.mark_safe(safe)
